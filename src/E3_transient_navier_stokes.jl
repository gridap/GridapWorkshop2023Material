...
# ## Numerical Scheme
#
# In order to approximate this problem we chose the same formulation as before, namely a formulation based on inf-sub stable $P_{k}/P_{k-1}$ triangular elements with continuous velocities and pressures. The interpolation spaces are defined as follows. The velocity interpolation space is
#
# $$
