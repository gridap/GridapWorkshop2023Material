...
# We then call the `amr_loop` function with `αr=0.1` and `αc=0.05` meaning that approximately 10% and 5% of the cells will approximately be refined and coarsened, respectively, at each adaptation step.  
...
# After execution, the function generates data visualization files along with the L2 and Hdiv errors of the discrete fluid flow and the L2 error of the discrete fluid pressure at each AMR level. At this point, we encourage the reader to open these files in ParaView and observe that, as expected, there is a clear tendency of the algorithm to adapt the mesh in the region where the circular wave front is located. The reader may also want to observe how the partition of the mesh among cells varies among levels in order to correct for the parallel load imbalances caused by mesh adaptation.

# As usual, it is helpful to visualize how the different errors decay with the number of degrees of freedom as the mesh is adapted across several adaptation cycles. The following code generates a plot and writes it into a file in the parallel task with identifier 0.
